...
/// # Errors
///
/// Returns an error if the secret cannot be resolved (env var missing/empty,
/// file unreadable/empty, or unrecognized reference scheme).
pub fn resolve(reference: &str) -> Result<String> {
    if let Some(var) = reference.strip_prefix("env:") {
        match std::env::var(var) {
            Ok(val) if !val.is_empty() => Ok(val),
            Ok(_) => bail!("Secret env:{var} is set but empty"),
            Err(_) => bail!("Secret env:{var} is not set"),
        }
        ...
        bail!("Invalid secret reference '{reference}'. Use 'env:<VAR>' or 'file:<path>'.",)
    }
}
...
/// # Errors
///
/// Returns an error if any secret in `pairs` cannot be resolved.
