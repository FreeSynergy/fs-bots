...
impl BotCommand for SyncStartCommand {
    ...
    fn usage(&self) -> Option<&str> {
        Some("sync-start <target_platform> <target_room>")
    }
...
impl BotCommand for SyncStopCommand {
    ...
    fn usage(&self) -> Option<&str> {
        Some("sync-stop <target_platform> <target_room>")
    }
    ...
    async fn execute(&self, ctx: CommandContext) -> BotResponse {
        ...
        match self
            .db
            .disable_rule(src_platform, src_room, tgt_platform, tgt_room)
            .await
        {
    ...
    async fn execute(&self, ctx: CommandContext) -> BotResponse {
        ...
        match self.db.active_rules_for(platform, room).await {
            ...
            Ok(rules) if rules.is_empty() => {
                BotResponse::text("No active sync rules for this room.")
            }
            Ok(rules) => {
